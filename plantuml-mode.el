...

;;; Groups and customize variables
(defgroup plantuml nil
  "Major mode for editing plantuml file."
  :group 'languages
  :prefix "plantuml-")

(defcustom plantuml-mode-hook nil
  "Hooks run by `plantuml'."
  :type 'hook
  :group 'plantuml)

(defcustom plantuml-jar-path nil
  "Java package of plantuml: `plantuml.jar'."
  :type 'string
  :group 'plantuml)

(defcustom plantuml-run-command "java -jar %s"
  "Java command and arguments to run `plantuml.jar'."
  :type 'string
  :group 'plantuml)
...

;; Constants
(defconst plantuml-mode-version "0.0.1" "Version of `plantuml-mode'.")



;;; Mode setup
(defvar plantuml-mode-map ()
  "Keymap used in `plantuml-mode' buffers.")

(if plantuml-mode-map
    nil
  (setq plantuml-mode-map (make-sparse-keymap))
  (define-key plantuml-mode-map (kbd "C-c C-c") #'plantuml-run-and-display))

(defvar plantuml-mode-syntax-table nil
  "Syntax table in use in `plantuml-mode' buffers.")

(if plantuml-mode-syntax-table
    nil
  (setq plantuml-mode-syntax-table (make-syntax-table))
  (modify-syntax-entry ?' "< b" plantuml-mode-syntax-table)
  (modify-syntax-entry ?\n "> b" plantuml-mode-syntax-table)
  (modify-syntax-entry ?! "w" plantuml-mode-syntax-table)
  (modify-syntax-entry ?@ "w" plantuml-mode-syntax-table)
  (modify-syntax-entry ?# "'" plantuml-mode-syntax-table))


;;;###autoload
(define-derived-mode plantuml-mode fundamental-mode "PlantUML"
  "Simple mode to edit PlantUML."
  :syntax-table plantuml-mode-syntax-table
